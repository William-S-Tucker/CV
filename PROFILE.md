...
- **Update 2026-09-24 (verified by survey agent against `C:\code\ascendai`, HEAD `5877bdc6`):** 3,050 commits (all one person across 7 git identities), 2026-04-25 → 2026-09-22. Ingestion pipeline now covers **8 jurisdictions** (`lib/bylaw-pipeline/`: fetch → extract → chunk 500/100 → Voyage embed ≤64/batch → pgvector, run by GitHub Actions cron), content-hash dedup, resumable ingest that skips already-embedded chunks, **blue-green chunk lifecycle** (live/pending/superseded) with transactional supersede-then-activate (`promote.ts`). **78 Supabase migrations**, ~45 tables, 46 RLS-enabled tables / 49 policies, HNSW tuned. `scripts/verify-zoning-citations.ts` cross-checks every YAML citation against corpus text and caught **395/395 wrong citations on Peachland** (ToC parse error). `document_rules_cache` keyed by `yaml_hash` + `bylaw_corpus_hash` with token-count telemetry; exponential backoff on Voyage 429s; per-user credit metering. **Hash-chained staff audit log** (`audit.staff_event`) with a daily digest published to CI logs, weekly retention dry-run with row-count floors. Multi-tenant `organization`/`org_membership`, email-OTP auth, Turnstile. 462 vitest files / ~4,500 cases; 3-leg CI matrix. Prod incident 2026-09-11: unhandled Postgres statement-timeout rejection crashed a shared Fluid Compute instance, root-caused and guarded in `instrumentation.ts`. **Status: 6-user pro-bono pilot, Stripe in test mode, no paying customers.** `PRODUCT.md` says "production-market build" but README says pilot; do not claim launched or revenue.
